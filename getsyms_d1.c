...
//argc is the Argument counte
    ...
    if(strcmp(argv[2], "RTLD_LAZY") == 0){ //Getting the section argument and checking the type using cmp function 
        ...
        fd = open("getsyms_d1_RTLD_LAZY.time.log", O_WRONLY | O_CREAT | O_APPEND, 0644);
    }

    if(strcmp(argv[2], "RTLD_NOW") ==0){
        dl_flagType = RTLD_NOW; 
        fd = open("getsyms_d1_RTLD_NOW.time.log", O_WRONLY | O_CREAT | O_APPEND, 0644);
    }
    ...
    time = ((float)(finish - start) / 2200000000) * 1000; //Get the diffrence to find the amount of time it took 
    ...
    *(void **) (&getsymbol_ptr) = dlsym(handle, "getsymb"); 
